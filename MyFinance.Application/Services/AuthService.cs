...
public class AuthService : IAuthService
{
    ...
    public async Task<(bool Success, LoginResponseDto? Data, string? ErrorMessage)> AuthenticateAsync(LoginRequestDto loginRequest)
    {
        ...
        {
            return (false, null, "Email ou senha inválidos."); // Critério de Aceitação
        }
    ...
    {
        // Verificar se o email já existe (Critério de Aceitação)
        ...
        // Criar o hash da senha (Critério de Aceitação)
        var passwordHash = BCrypt.Net.BCrypt.HashPassword(registerRequest.Password);

        // Criar a nova entidade User
        var newUser = new User
        {
            Id = Guid.NewGuid(), // Gerar um novo ID único
    ...
    private string GenerateJwtToken(User user)
    {
        ...
        // Criar as "claims" (informações dentro do token)
        var claims = new[]
        {

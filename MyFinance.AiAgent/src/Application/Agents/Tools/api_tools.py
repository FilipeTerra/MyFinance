...
# =============================================================================
# Erros de acesso ao backend .NET + decorator de tratamento
#
# Antes, cada ferramenta repetia o mesmo bloco (401 / offline / inesperado) no
# fim do corpo. Agora _get_json/_post_json levantam exceções tipadas e o
# decorator @_handle_api_errors as traduz na mensagem apropriada — string (tools
# de chat) ou dict {"erro": ...} (tools que devolvem dados estruturados).
#
# IMPORTANTE: só erros de TRANSPORTE/autorização passam por aqui. Erros de
# NEGÓCIO (ex: "usuário sem renda cadastrada") continuam sendo `return` explícito
# dentro da ferramenta e trafegam intactos.
# =============================================================================

class _ApiError(Exception):
    """Falha de transporte/autorização ao acessar o backend .NET."""

class _SessionExpired(_ApiError):
    """JWT ausente, expirado ou inválido (HTTP 401)."""

class _ApiOffline(_ApiError):
    """Backend .NET inacessível (rede/porta)."""

class _ApiStatus(_ApiError):
...
def _handle_api_errors(as_dict: bool = False):
    ...
    def deco(fn):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                return _err(_ERR_SESSAO)
            except (_ApiOffline, httpx.RequestError) as e:
                _logger.warning("🔌 [API] Backend .NET offline em %s: %s", fn.__name__, e)
                return _err(_ERR_OFFLINE)
            except _ApiStatus as e:
...
def make_api_tools(jwt_token: str) -> list:
    ...
    async def _get_json(path: str):
        """GET no .NET; levanta _ApiOffline/_SessionExpired/_ApiStatus e devolve o JSON."""
        try:
            r = await _client.get(f"{_API_BASE_URL}{path}")
        except httpx.RequestError as e:
            raise _ApiOffline(str(e)) from e
        if r.status_code == 401:
            raise _SessionExpired()
        if r.status_code != 200:
            raise _ApiStatus(r.status_code, r.text)
        return r.json()
    ...
    async def _post_json(path: str, payload: dict):
        ...
        try:
            r = await _client.post(f"{_API_BASE_URL}{path}", json=payload)
        except httpx.RequestError as e:
            raise _ApiOffline(str(e)) from e
        if r.status_code == 401:
            raise _SessionExpired()
    ...
    async def _buscar_todas_transacoes(
        ...
        """
        contas = await _get_json("/accounts") or []
        if not contas:
    ...
    async def consultar_metas_financeiras() -> str:
        ...
        metas = await _get_json("/financial-goals") or []
        if not metas:
    ...
    async def criar_meta_financeira(nome: str, valor_alvo: float, data_limite: str) -> str:
        ...
        await _post_json("/financial-goals", payload)
    ...
    async def realizar_aporte_meta(valor: float, goal_id: str, account_id: str) -> str:
        ...
        categorias = await _get_json("/categories") or []
        if not categorias:
        ...
        await _post_json("/transactions", payload)
    ...
    @_handle_api_errors(as_dict=True)
    ...
    async def consultar_perfil_usuario() -> str:
        ...
        perfil = await _get_json("/Profile") or {}
    ...
    async def consultar_investimentos() -> str:
        ...
        investimentos = await _get_json("/investimentos") or []
